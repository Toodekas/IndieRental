...
import indiegame_store.ui.order.OrderListLogic;
...
public class OrderForm extends Div {
    ...
    private OrderedVideoGamess orderedGames;
    ...
    public OrderForm(OrderListLogic orderListLogic) {
        ...
        orderedGames = new OrderedVideoGamess();
        content.add(orderedGames);
        ...
        binder.forField(orderDate)
                .bind("orderDate");
        binder.forField(orderedGames)
                .withValidator(items -> items != null && items.size() > 0, "Add at least one game")
                .bind("items");
        ...
        returnButton = new Button("Return games");
        returnButton.setId("return");
        returnButton.setWidth("100%");
        returnButton.addClickListener(event -> {
            ReturnGameWindow returnGameWindow = new ReturnGameWindow(currentOrder, orderListLogic.getOrderToReceiptService(), viewLogic.getRepository(), () -> viewLogic.editOrder(currentOrder));
            returnGameWindow.open();
        });
    ...
    public void editOrder(RentOrder order) {
        ...
        if (isNew) {
            order = new RentOrder();
            orderedGames.setReadOnly(false);
            delete.setVisible(false);
            returnButton.setVisible(false);
        } else {
            orderedGames.setReadOnly(true);
        ...
        // TODO: returnButton should be not enabled if all games were returned from this order
        returnButton.setEnabled(true);

        // TODO: Delete button should be disabled during new order creation or if order there is not all games returned.
